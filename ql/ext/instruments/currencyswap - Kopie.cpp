...
ResetableCrossCurrencySwap::ResetableCrossCurrencySwap(bool payDom, const Currency& ccyDom, Real nominalDomInitial, const Schedule& scheduleDom,
	const boost::shared_ptr<IborIndex>& iborIndexDom, std::vector<Rate> spreadsDom,
	...
	: CurrencySwap(4), scheduleFor_(scheduleFor), scheduleDom_(scheduleDom), fxIndex_(fxIndex),
	  nominalsFor_(nominalsFor) {

	//, nominalsDom_(std::vector<Real>(nominalsFor.size(), nominalDomInitial))
	// outdated_ = false;
	std::vector<Real> nominalsDom_(nominalsFor_.size(), nominalDomInitial);
	...
	currency_[2] = ccyFor;
	payer_[2] = (payDom ? +1 : -1);
	legs_[2] = IborLeg(scheduleFor_, iborIndexFor)
		.withNotionals(nominalsFor_)
		.withPaymentDayCounter(iborIndexFor->dayCounter())
		.withPaymentAdjustment(convention_)
		.withSpreads(spreadsFor);
	for (Leg::const_iterator i = legs_[2].begin(); i < legs_[2].end(); ++i)
		registerWith(*i);
