...
OvernightIndexedBasisMtMResetSwap::OvernightIndexedBasisMtMResetSwap(
    Real foreignNominal, const Currency& foreignCurrency, const Schedule& foreignSchedule,
    const boost::shared_ptr<OvernightIndex>& foreignIndex, Spread foreignSpread, const Currency& domesticCurrency,
    const Schedule& domesticSchedule, const boost::shared_ptr<OvernightIndex>& domesticIndex, Spread domesticSpread,
    const boost::shared_ptr<FxIndex>& fxIdx, bool invertFxIdx, bool receiveDomestic)
    : CrossCcySwap(3), foreignNominal_(foreignNominal), foreignCurrency_(foreignCurrency),
      ...
      domesticSpread_(domesticSpread), fxIndex_(fxIdx), invertFxIndex_(invertFxIdx), receiveDomestic_(receiveDomestic) {
...
void OvernightIndexedBasisMtMResetSwap::initialize() {
    // Pay (foreign) leg
    legs_[0] = OvernightLeg(foreignSchedule_, foreignIndex_).withNotionals(foreignNominal_).withSpreads(foreignSpread_);
    ...
    legs_[1] = OvernightLeg(domesticSchedule_, domesticIndex_).withNotionals(0).withSpreads(domesticSpread_);
...
void OvernightIndexedBasisMtMResetSwap::setupArguments(PricingEngine::arguments* args) const {

    CrossCcySwap::setupArguments(args);

    OvernightIndexedBasisMtMResetSwap::arguments* arguments = dynamic_cast<OvernightIndexedBasisMtMResetSwap::arguments*>(args);
...
void OvernightIndexedBasisMtMResetSwap::fetchResults(const PricingEngine::results* r) const {

    CrossCcySwap::fetchResults(r);

    const OvernightIndexedBasisMtMResetSwap::results* results = dynamic_cast<const OvernightIndexedBasisMtMResetSwap::results*>(r);
    if (results) {
...
void OvernightIndexedBasisMtMResetSwap::setupExpired() const {
    CrossCcySwap::setupExpired();
    fairForeignSpread_ = Null<Spread>();
    fairDomesticSpread_ = Null<Spread>();
}

void OvernightIndexedBasisMtMResetSwap::arguments::validate() const {
...
void OvernightIndexedBasisMtMResetSwap::results::reset() {
