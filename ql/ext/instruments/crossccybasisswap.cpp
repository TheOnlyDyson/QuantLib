...
void CrossCcyBasisSwap::initialize() {
    // Pay leg
    legs_[0] = IborLeg(paySchedule_, payIndex_).withNotionals(payNominal_).withSpreads(paySpread_);
    payer_[0] = -1.0;
    ...
    // Receive leg
    legs_[1] = IborLeg(recSchedule_, recIndex_).withNotionals(recNominal_).withSpreads(recSpread_);
    payer_[1] = +1.0;
