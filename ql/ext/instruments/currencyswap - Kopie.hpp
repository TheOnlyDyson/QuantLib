...
public:
	...
	// floating/floating
	CrossCurrencySwap(bool pay1, const Currency& ccy1, std::vector<Real> nominals1, const Schedule& schedule1,
		const boost::shared_ptr<IborIndex>& iborIndex1, std::vector<Rate> spreads1, const Currency& ccy2,
...
public:
	...
	void update() { legs_[1] = updateDomLegFlows(); notifyObservers(); }
...
private:
	// bool outdated_;
	Leg updateDomLegFlows();

	const Schedule& scheduleFor_;
	const Schedule& scheduleDom_;
	const boost::shared_ptr<FxIndex>& fxIndex_;
...
inline Leg ResetableCrossCurrencySwap::updateDomLegFlows() {
	Leg fxLeg;
	...
	fxLeg.push_back(boost::shared_ptr<CashFlow>(
	...
	for (Size i = 1; i < nominalsDom_.size(); i++) {
		//fxLeg.push_back(boost::shared_ptr<CashFlow>(
		//	new FXLinkedCashFlow(scheduleDom_.calendar().adjust(scheduleDom_[i], convention_), scheduleDom_.calendar().advance(scheduleDom_[i], -1 * fxIndex_->fixingDays(), TimeUnit::Days, Preceding), nominalsFor_[i - 1] - nominalsFor_[i], fxIndex_)));
