...
void SubPeriodsCouponPricer::initialize(const FloatingRateCoupon& coupon) {
    ...
    coupon_ = dynamic_cast<const SubPeriodsCoupon_ext*>(&coupon);
    QL_REQUIRE(coupon_, "SubPeriodsCoupon_ext required");
...
Rate SubPeriodsCouponPricer::swapletRate() const {
    ...
    if (type_ == SubPeriodsCoupon_ext::Averaging) {
        accumulatedRate = 0.0;
    ...
    } else if (type_ == SubPeriodsCoupon_ext::Compounding) {
        accumulatedRate = 1.0;
        for (Size i = 0; i < numPeriods; ++i) {
