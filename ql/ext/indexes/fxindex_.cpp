...
Real FxIndex::fixing(const Date& fixingDate, bool forecastTodaysFixing, Date& today) const {
...
Real FxIndex::forecastFixing(const Date& fixingDate, const Date& today) const {
	...
	if (today == Date()) {
		...
		Date refValueDate = valueDate(today);
	}
