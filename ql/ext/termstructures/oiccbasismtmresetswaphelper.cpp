...
OvernightIndexedBasisMtMResetSwapHelper::OvernightIndexedBasisMtMResetSwapHelper(
    ...
    const boost::shared_ptr<OvernightIndex>& foreignCcyIndex,
    const boost::shared_ptr<OvernightIndex>& domesticCcyIndex,
    ...
    QL_REQUIRE(foreignCurrency_ != domesticCurrency_,
               "matching currencies not allowed on OvernightIndexedBasisMtMResetSwapHelper");
    ...
    QL_REQUIRE(
        ...
        "OvernightIndexedBasisMtMResetSwapHelper - Have all curves, nothing to solve for.");
    ...
    if (foreignIndexHasCurve && haveForeignDiscountCurve) {
        if (!domesticIndexHasCurve) {
            domesticCcyIndex_ = boost::dynamic_pointer_cast<OvernightIndex>( domesticCcyIndex_->clone(termStructureHandle_) );
        ...
        if (!foreignIndexHasCurve) {
            foreignCcyIndex_ = boost::dynamic_pointer_cast<OvernightIndex>( foreignCcyIndex_->clone(termStructureHandle_) );
...
void OvernightIndexedBasisMtMResetSwapHelper::initializeDates() {
    ...
    swap_ = boost::shared_ptr<OvernightIndexedBasisMtMResetSwap>(new OvernightIndexedBasisMtMResetSwap(
...
void OvernightIndexedBasisMtMResetSwapHelper::setTermStructure(YieldTermStructure* t) {
...
Real OvernightIndexedBasisMtMResetSwapHelper::impliedQuote() const {
...
void OvernightIndexedBasisMtMResetSwapHelper::accept(AcyclicVisitor& v) {
    Visitor<OvernightIndexedBasisMtMResetSwapHelper>* v1 = dynamic_cast<Visitor<OvernightIndexedBasisMtMResetSwapHelper>*>(&v);
    if (v1)
        v1->visit(*this);
    else
        RateHelper::accept(v);
